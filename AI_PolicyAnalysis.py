...
with open('report.txt', 'w') as report:
    for index, row in resumes.iterrows():
        text = clean_text(row['resume_text'])  # Update to use the correct column name
...
# Calculate top words and bigrams for china
top_words_china = calculate_top_words(text_china)
top_bigrams_china = calculate_bigrams(text_china)

# Load and preprocess text data for EU and US
text_europe = read_and_preprocess_text('EU.txt')
text_us = read_and_preprocess_text('US.txt')

#printing top words and bigrams for china
print("Top Words in China:")
for word, freq in top_words_china:
    print(f"{word}: {freq}")

print("\nTop Bigrams in China:")
for bigram, freq in top_bigrams_china:
    print(f"{bigram}: {freq}")

# Calculate top words and bigrams for EU 
top_words_europe = calculate_top_words(text_europe)
top_bigrams_europe = calculate_bigrams(text_europe)

# Print Top Words and Bigrams for EU
print("Top Words in Europe:")
for word, freq in top_words_europe:
    print(f"{word}: {freq}")

print("\nTop Bigrams in Europe:")
for bigram, freq in top_bigrams_europe:
    print(f"{bigram}: {freq}")

# Calculate top words and bigrams for US 
top_words_us = calculate_top_words(text_us)
top_bigrams_us = calculate_bigrams(text_us)


# Print Top Words and Bigrams for the US
print("\nTop Words in US:")
for word, freq in top_words_us:
    print(f"{word}: {freq}")

print("\nTop Bigrams in US:")
for bigram, freq in top_bigrams_us:
    print(f"{bigram}: {freq}")
...
generate_word_cloud(text_china, 'China Word Cloud')
generate_word_cloud(text_europe, 'Europe Word Cloud')
generate_word_cloud(text_us, 'US Word Cloud')
